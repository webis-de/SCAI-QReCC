...
"""Calculates the measures for the SCAI QReCC 22 challenge"""
# Version: 2022-06-28
...
def parse_options():
    ...
    for opt, arg in opts:
        ...
        if opt == "--input-run":
            ...
            runs = glob(arg + "/*.json")
            if len(runs) != 1:
                sys.exit("I expect exactly one json file in the input run folder. Got: " + str(runs))
            
            options["run"] = load_run(runs[0])
